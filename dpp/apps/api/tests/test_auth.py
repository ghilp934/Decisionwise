...
def test_api_key_repository_create(db_session: Session):
    ...
    # Create tenant first
    tenant = Tenant(
        tenant_id=f"tenant_api_key_test_{uuid.uuid4().hex[:8]}",
...
def test_api_key_repository_get_active(db_session: Session):
    ...
    # Create tenant
    tenant = Tenant(
        tenant_id=f"tenant_active_test_{uuid.uuid4().hex[:8]}",
        display_name="Test Tenant",
        status="ACTIVE",
    )
    db_session.add(tenant)
...
def test_api_key_repository_update_last_used(db_session: Session):
    ...
    tenant = Tenant(
        tenant_id=f"tenant_last_used_{uuid.uuid4().hex[:8]}",
        display_name="Test Tenant",
        status="ACTIVE",
    )
    db_session.add(tenant)
...
def test_api_key_repository_revoke(db_session: Session):
    ...
    # Create tenant and key
    tenant = Tenant(
        tenant_id=f"tenant_revoke_{uuid.uuid4().hex[:8]}",
        display_name="Test Tenant",
        status="ACTIVE",
    )
    db_session.add(tenant)
