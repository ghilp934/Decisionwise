...
def test_tenant(db_session: Session) -> Tenant:
    """Create a test tenant (cleanup existing if present)."""
    tenant_id = "tenant_monetization_test"

    # Delete existing tenant if present (PostgreSQL persistence)
...
def basic_plan(db_session: Session) -> Plan:
    """Create a basic plan with limits (cleanup existing)."""
    plan_id = "plan_basic"
...
def premium_plan(db_session: Session) -> Plan:
    """Create a premium plan with higher limits (cleanup existing)."""
...
def test_usage_metering_accuracy(
    ...
    # Cleanup any existing data for today (from previous tests)
    db_session.query(TenantUsageDaily).filter_by(
        tenant_id=test_tenant.tenant_id, usage_date=today
    ).delete()
    db_session.query(Run).filter_by(tenant_id=test_tenant.tenant_id).delete()
    db_session.commit()
